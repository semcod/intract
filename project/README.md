...
- `evolution.toon.yaml` (**YAML**) — **📋 Refactoring queue**: prioritized improvements; key insight: 0 refactoring actions needed.
- `map.toon.yaml` (**YAML**) — **🗺️ Structural map + project header**: modules, imports, exports, signatures, stats, alerts, hotspots, trend; key insight: project architecture overview.

### 🤖 LLM-Ready Documentation

- `prompt.txt` (**Text**) — **📝 Ready-to-send prompt** listing all files with instructions; use case: attach to LLM conversation as context guide.
- `context.md` (**Markdown**) — **📖 LLM narrative** with architecture summary; use case: paste into ChatGPT/Claude for code analysis.
...
**Purpose**: Run the most common baseline analysis commands.
...
**Purpose**: Optimize analysis speed and resource usage.
...
**Purpose**: Generate refactoring-oriented outputs and diagnostics.
...
### Generate Intract Toon Manifest from `map.toon.yaml`

**Purpose**: Generate and validate Intract manifests from `code2llm` map output.

```bash
# 1) Generate project/file/function URI contracts from code2llm map
python scripts/generate_toon_from_map.py \
  --map project/map.toon.yaml \
  --output project/generated.intract.toon

# 2) Validate on demand against current workspace
python -m intract validate . --manifest project/generated.intract.toon
```

#### LLM Enrichment (optional)

**Purpose**: Enrich function-level contracts with LLM proposals.

```bash
# Requires OPENROUTER_API_KEY and litellm extra (pip install -e .[llm])
python scripts/generate_toon_from_map.py \
  --map project/map.toon.yaml \
  --output project/generated.intract.toon \
  --llm \
  --llm-goal "focus on security in CI, function behavior in dev"
```

#### Output Profiles (`--output-profile`)

**Purpose**: Switch generated contract focus for development or CI security stages.

```bash
# Dev profile: no project contract, function contracts focused on return_value
python scripts/generate_toon_from_map.py \
  --map project/map.toon.yaml \
  --output project/generated.intract.dev.toon \
  --output-profile dev

# CI security profile: includes project contract, function contracts focused on no_forbidden_effect
python scripts/generate_toon_from_map.py \
  --map project/map.toon.yaml \
  --output project/generated.intract.ci-security.toon \
  --output-profile ci-security \
  --validate --root .
```

Recommended usage:

- `dev`: during feature development when focus is function behavior and return values.
- `ci-security`: in CI/CD gates when focus is policy/security and forbidden effects.
...
**Purpose**: Quickly inspect health and refactoring indicators.
...
**Purpose**: Build a focused refactoring plan from generated artifacts.
...
**Purpose**: Prepare compact context for AI-assisted refactoring.
...
**Purpose**: Produce shared documentation artifacts for teams.
...
**Purpose**: Enforce quality checks in CI.
...
**Purpose**: Catch critical issues before commits.
...
**Purpose**: Update project docs from generated context.
...
**Purpose**: Run advanced and deep analysis scenarios.
...
**Purpose**: Control output layout and artifact structure.
